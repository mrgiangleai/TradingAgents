...
## Phase 7 — Final Advisor (thiết kế → cài đặt → test tổ hợp)

**Ngày chạy:** 2026-07-10
**Memory path dùng:** `~/.tradingagents/memory/test_memory.md` (Quy tắc 1)

### Quyết định kiến trúc bắt buộc (yêu cầu chính của phiên này)

**Final Advisor BỔ SUNG, không thay thế Portfolio Manager.** Node mới `Portfolio Manager -> Final Advisor -> END` (thay `Portfolio Manager -> END` cũ), ghi vào field state MỚI `final_advisory_report` — **không đổi** `final_trade_decision`, `risk_debate_state`, hay bất kỳ field nào `memory_log`/`SignalProcessor`/`_log_state` đang đọc. Lý do đầy đủ + phương án bị loại: `docs/agents/final_advisor_design.md` mục 1.

Final Advisor chỉ đọc: `final_trade_decision` (luôn có, PM không toggle được) + `keyvolume_report`/`liquidity_sweep_report` (có thể vắng mặt/`no_data`/tín hiệu thật). **Không đọc lại** 4 report gốc hay debate history (đúng quy ước "mỗi lớp tổng hợp chỉ đọc lớp ngay trước" đã có sẵn trong hệ thống — Trader/Portfolio Manager cũng không đọc report gốc).

**Không có cờ bật/tắt riêng cho Final Advisor** — luôn chạy (deliverable chính của Phase 7, khác KeyVolume/Liquidity Sweep là tín hiệu tuỳ chọn).

### File bị sửa

| File | Lý do |
|---|---|
| `tradingagents/agents/schemas.py` | `FinalAdvisoryReport` (tái dùng `PortfolioRating`, không tạo enum mới) + `render_final_advisory_report` (**không** chèn dòng disclaimer ở đây — xem lý do bên dưới). |
| `tradingagents/agents/managers/final_advisor.py` (mới) | Node `create_final_advisor(llm)`, dùng đúng `bind_structured`/`invoke_structured_or_freetext` như Portfolio Manager. Tự chèn dòng `"**Advisory only -- not an automated trade order.**"` **sau khi** gọi `invoke_structured_or_freetext` — vì hàm này ở nhánh fallback free-text trả thẳng `response.content`, bỏ qua hàm `render`, nên disclaimer đặt trong `render_final_advisory_report` sẽ **không** được đảm bảo xuất hiện ở nhánh đó. Đặt disclaimer ở node (áp dụng cho cả 2 nhánh) mới đúng Quy tắc 3. |
| `tradingagents/agents/__init__.py` | Export `create_final_advisor` (theo đúng pattern `create_portfolio_manager`). |
| `tradingagents/graph/setup.py` | Thêm node + 2 cạnh (`Portfolio Manager -> Final Advisor -> END`), dùng `deep_thinking_llm` (giống Research Manager/Portfolio Manager — cũng là node tổng hợp/phán đoán cuối, không phải tác vụ máy móc). |
| `tradingagents/agents/utils/agent_states.py` | Field `final_advisory_report` (bắt buộc — LangGraph định nghĩa channel theo `AgentState` schema). |
| `tradingagents/reporting.py` | Mục "VIII. Final Advisor (Advisory Only)", `8_final_advisor/`, thuần cộng thêm sau mục VII, không renumber. |

**Không sửa:** Portfolio Manager, Trader, Research Manager, Bull/Bear, Risk debator, `analyst_execution.py`, `propagation.py`, `_log_state` (Phase 8), KeyVolume/Liquidity Sweep Agent's code, Backtest-Trading-Lab.

### Test 1 — Cấu trúc graph (không tốn API)

- ✅ Node "Final Advisor" luôn có mặt trong graph bất kể cấu hình analyst/KeyVolume/Liquidity Sweep nào (không có cờ riêng).

### Test 2 — `write_report_tree` (không qua LLM)

- ✅ Tất cả field có mặt (`keyvolume_report`, `liquidity_sweep_report`, `final_advisory_report`) → tạo đủ `6_keyvolume/`, `7_liquidity_sweep/`, `8_final_advisor/`, cả 3 mục VI/VII/VIII xuất hiện trong `complete_report.md`, dòng disclaimer có mặt.
- ✅ Re-test fixture y hệt `tests/test_reporting.py` (5 section cũ) → không regression, không tạo `8_final_advisor/` khi field vắng mặt.

### Test 3 — Full pipeline thật, đủ tổ hợp yêu cầu (memory test)

| Case | Ticker/ngày | `keyvolume_report` | `liquidity_sweep_report` | `final_advisory_report` (tóm tắt) | Crash? | Thời gian |
|---|---|---|---|---|---|---|
| **Cả 2 ON** (có dữ liệu thật) | BTCUSDT / 2026-07-09 | neutral/medium (thật) | bullish/medium (thật) | Overweight/high — tổng hợp đúng cả 2 tín hiệu + PM decision, có disclaimer | ❌ | 90.1s |
| **Chỉ KeyVolume ON** (thiếu file) | LINKUSDT / 2026-07-09 | `no_data` | Vắng mặt (tắt) | Buy/medium — nêu rõ "Both the KeyVolume signal and the Liquidity Sweep were unavailable", không đoán | ❌ | 49.6s |
| **Chỉ Liquidity Sweep ON** (có dữ liệu thật) | DOGEUSDT / 2026-07-09 | Vắng mặt (tắt) | bullish/medium (thật) | Underweight/medium — nêu rõ "the KeyVolume signal was disabled for this run", vẫn ưu tiên PM decision khi xung đột | ❌ | 51.8s |
| **Cả 2 OFF** (mặc định) | AVAXUSDT / 2026-07-09 | Vắng mặt | Vắng mặt | Hold/medium — nêu rõ "Both ... were not enabled for this run" | ❌ | 82.4s |
| **Cả 2 ON, cả 2 thiếu dữ liệu** | DOTUSDT / 2026-07-09 | `no_data` | `no_data` | Underweight/medium — nêu rõ "Both KeyVolume and Liquidity Sweep signals returned no_data" | ❌ | 47.9s |

Cả 5 case: `final_advisory_report` **luôn** sinh ra, luôn có dòng `"**Advisory only -- not an automated trade order.**"`, `final_trade_decision` không đổi hành vi so với trước Phase 7. Không case nào model tự bịa số liệu cho tín hiệu vắng mặt/`no_data` — luôn nêu rõ "unavailable"/"disabled"/"no_data" bằng chính chữ trong report.

### Kiểm chứng cách ly memory (Quy tắc 1)

- ✅ `test_memory.md` có thêm 5 entry mới (BTCUSDT, LINKUSDT, DOGEUSDT, AVAXUSDT, DOTUSDT — tổng 18 entry `ENTRY_END`).
- ✅ `trading_memory.md` (memory thật) — vẫn không tồn tại.

### Điều kiện hoàn thành Phase 7

- [x] Xác định rõ Final Advisor bổ sung (không thay thế) Portfolio Manager, có lý do ghi rõ.
- [x] Structured output (`FinalAdvisoryReport`, tái dùng `PortfolioRating`).
- [x] KeyVolume/Liquidity Sweep tắt hoặc thiếu dữ liệu → vẫn chạy bình thường, không crash.
- [x] Không tự bịa dữ liệu còn thiếu — xác nhận bằng text thật ở cả 5 case.
